...
def main():
    parser = argparse.ArgumentParser(
        description="cf-deployer: Deploy CloudFormation stacks across multiple environments and teams"
    )
    parser.add_argument(
        "command", choices=["deploy"], help="Command to execute"
    )
    parser.add_argument(
        "-e", "--env", required=True, help="Environment to deploy (dev/test/prod)"
    )
    parser.add_argument(
        "-t", "--team", help="Specific team to deploy (optional)"
    )
    parser.add_argument(
        "-s", "--stack", help="Specific stack to deploy (optional)"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be deployed without executing"
    )
    parser.add_argument(
